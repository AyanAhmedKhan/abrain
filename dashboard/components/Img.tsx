...
export function Avatar({ src, name, size = 28 }: { src?: string | null; name?: string; size?: number }) {
  ...
  if (url && !err) {
    ...
    return <img src={url} alt="" style={style} onError={() => setErr(true)}
                className="rounded-full object-cover bg-cream shrink-0" />;
  }
  return <span style={style} className="rounded-full bg-cream text-dim inline-flex items-center justify-center shrink-0">{initial(name)}</span>;
}
...
export function Logo({ src, name, size = 36 }: { src?: string | null; name?: string; size?: number }) {
  ...
  if (url && !err) {
    // eslint-disable-next-line @next/next/no-img-element
    return <img src={url} alt="" style={style} onError={() => setErr(true)}
                className="rounded object-contain bg-cream shrink-0" />;
  }
  return <span style={style} className="rounded bg-cream text-dim inline-flex items-center justify-center font-semibold shrink-0">{initial(name)}</span>;
}
